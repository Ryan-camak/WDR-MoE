...
class DASA(nn.Module):
    """Dual-Attention Selective Aggregator adapted from FLoMo-Net."""

    def __init__(self, c1: int, c2: int, reduction_ratio: int = 8, kernel_size: int = 7, num_freq_components: int = 16):
        """Initialize DASA block.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            reduction_ratio (int): Channel reduction ratio for the MLP gate.
            kernel_size (int): Spatial attention kernel size.
            num_freq_components (int): Number of low-frequency FFT bins used.
        """
        super().__init__()
        self.proj = Conv(c1, c2, 1, 1) if c1 != c2 else nn.Identity()
        self.num_freq_components = num_freq_components
        self._dct_basis_cache: dict[tuple[int, int], torch.Tensor] = {}

        reduced = max(c2 // max(reduction_ratio, 1), 8)
        self.channel_norm = nn.LayerNorm(c2 * 5)
        self.channel_mlp = nn.Sequential(
            nn.Linear(c2 * 5, reduced, bias=True),
            nn.ReLU(inplace=True),
            nn.Linear(reduced, c2, bias=True),
            nn.Sigmoid(),
        )

        self.spatial_weights = nn.Parameter(torch.tensor([0.5, 0.5], dtype=torch.float32))
        self.spatial_dw = nn.Conv2d(1, 1, kernel_size=kernel_size, stride=1, padding=autopad(kernel_size), bias=False)
        self.spatial_pw = nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0, bias=True)

    def _get_dct_basis(self, n: int, k: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        """Get orthonormal DCT-II basis [k, n], cached by shape/device/dtype."""
        key = (n, k)
        if key in self._dct_basis_cache:
            basis = self._dct_basis_cache[key]
            if basis.device != device or basis.dtype != dtype:
                basis = basis.to(device=device, dtype=dtype)
                self._dct_basis_cache[key] = basis
            return basis

        nn_idx = torch.arange(n, device=device, dtype=dtype) + 0.5
        kk_idx = torch.arange(k, device=device, dtype=dtype).unsqueeze(1)
        basis = torch.cos((math.pi / n) * kk_idx * nn_idx)
    ...
    def _low_freq_stats(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Compute low-frequency avg/max descriptors from explicit 2D DCT."""
        h_in, w_in = x.shape[-2], x.shape[-1]
        h = min(self.num_freq_components, h_in)
        w = min(self.num_freq_components, w_in)

        x_dct = x.float()
        bh = self._get_dct_basis(h_in, h, x.device, x_dct.dtype)
        bw = self._get_dct_basis(w_in, w, x.device, x_dct.dtype)
        low = torch.einsum("bchw,kh,qw->bckq", x_dct, bh, bw)

        avg = F.adaptive_avg_pool2d(low, 1).flatten(1).to(x.dtype)
        mx = low.amax(dim=(2, 3)).to(x.dtype)
        return avg, mx

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply channel attention followed by adaptive weighted spatial attention."""
        x = self.proj(x)
        avg_orig = F.adaptive_avg_pool2d(x, 1).flatten(1)
        max_orig = x.amax(dim=(2, 3))
        avg_freq, max_freq = self._low_freq_stats(x)

        pooled = F.avg_pool2d(x, kernel_size=4, stride=2, padding=1)
        pooled = F.avg_pool2d(pooled, kernel_size=4, stride=2, padding=1)
        inter_avg = F.adaptive_avg_pool2d(pooled, 1).flatten(1)

        channel_desc = torch.cat((avg_orig, max_orig, avg_freq, max_freq, inter_avg), dim=1)
        channel_gate = self.channel_mlp(self.channel_norm(channel_desc)).unsqueeze(-1).unsqueeze(-1)
        x = x * channel_gate

        avg_map = x.mean(dim=1, keepdim=True)
        max_map = x.amax(dim=1, keepdim=True)
        w = self.spatial_weights.softmax(dim=0)
        spatial_map = w[0] * avg_map + w[1] * max_map
        spatial_map = torch.sigmoid(self.spatial_pw(self.spatial_dw(spatial_map)))
