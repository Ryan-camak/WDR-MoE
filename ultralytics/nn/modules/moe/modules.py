...
from .experts import (
    ...
    InvertedResidualExpert, EfficientExpertGroup, SpatialExpert, WaveletFrequencyExpert
)
from .routers import (
...
class OptimizedMOEImproved(nn.Module):
    ...
    def __init__(
            ...
            num_experts: int = 4,
            top_k: int = 2,
            expert_type: str = 'simple',  # ['simple', 'ghost', 'inverted', 'spatial', 'wavelet']
        ...
        if expert_type == 'ghost':
            ...
            expert_cls = WaveletFrequencyExpert
            kwargs['expand_ratio'] = expert_expand_ratio
        else:
            expert_cls = SimpleExpert
            kwargs['expand_ratio'] = expert_expand_ratio

        if expert_type == 'wavelet':
            band_order = ('ll', 'lh', 'hl', 'hh')
            for i in range(num_experts):
                band = band_order[i % len(band_order)]
                self.experts.append(expert_cls(in_channels, out_channels, frequency_band=band, **kwargs))
        else:
            for _ in range(num_experts):
